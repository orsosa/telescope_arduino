...
void imudof::readData()
{
    ...
    kGyro[0] = (((int16_t)data[0]) << 8) | data[1]; //x
    kGyro[1] = (((int16_t)data[2]) << 8) | data[3]; //y : Esta variable se usara para determinar el angulo del servo
                                                    //    Cuando el eje Y esta horizontal tira valor 0, a los 90° tira 16384
    kGyro[2] = (((int16_t)data[4]) << 8) | data[5]; //z

    kAccel[0] = (((int16_t)data[8])  << 8) | data[9]; //x
    kAccel[1] = (((int16_t)data[10]) << 8) | data[11];//y
    kAccel[2] = (((int16_t)data[12]) << 8) | data[13];//z
    ...
    for (int k=0;k<3;k++)
    {
      kAccelF[k] = (float) kAccel[k]/ 16384;
