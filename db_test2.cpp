...
ISR(TIMER3_OVF_vect)
{
	...
	if(tot_overflow<0)                                                     // Condition for send the value (gate time is reached)
	{
		...
		uart1_send("AT+CIPSTART=4,\"TCP\",\"192.168.1.212\",80");       // Gate time works like the delay
		uart_send(uart1_read_cmd());
...
void prog_init()      //  Initialization function ...
{
	...
	uart1_send("AT+CIPSTART=4,\"TCP\",\"192.168.1.212\",80");       // Port to Send Data
	_delay_ms(500);
	uart1_send("AT+CIPSTART=3,\"TCP\",\"192.168.1.212\",80");       // Port to Read Dara
	_delay_ms(500);
	uart_send("ready");
}
